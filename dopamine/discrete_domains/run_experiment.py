...
def create_agent(sess, environment, agent_name=None, summary_writer=None,
  ...
  if agent_name == 'dqn':
    ...
    return implicit_quantile_agent.ImplicitQuantileAgent(
        sess, num_actions=environment.action_space.n, environment=environment, #CHANGE
        summary_writer=summary_writer)
  elif agent_name == 'fqf':
...
class Runner(object):
  ...
  def __init__(self,
    ...
    self.opponent.load_model('/home/neriait/tic_tac_toe_IQN/dopamine/discrete_domains/tictactoe/examples/mirai')
  ...
  def _create_directories(self):
    ...
    self._checkpoint_dir = os.path.join(self._base_dir, 'checkpointsrandom')
  ...
  def _initialize_episode(self, run_mode_str):
    ...
    return self._agent.get_action_from_observation(initial_observation), initial_observation
  ...
  def switch_tictacoe_observation_sides(self,observation):
  ...
  def _run_one_episode(self, run_mode_str):
    ...
    action, switchedobservation2 = self._initialize_episode(run_mode_str)
    is_terminal = False
    ...
    while True:
      if False:
        observation, reward, is_terminal = self._run_one_step(random.choice(self._environment.available_actions()))
      else:
        observation, reward, is_terminal = self._run_one_step(action) 
      
      switchedobservation=self.switch_tictacoe_observation_sides(deepcopy(list(observation[0])))
      if not is_terminal:
        if run_mode_str=='evalrandom':
          ...
          observation2, reward2, is_terminal = self._run_one_step(self.opponent.act(observation, self._environment.available_actions()))
          action2=None
        else:
          action2=self._agent.get_action_from_observation(observation[0])
        ...
        gameplayer.append([list(switchedobservation2),reward, is_terminal,action])
        switchedobservation2=list(observation2[0])
        gameopponent.append([observation[0],-reward, is_terminal,action2])
      ...
      if (is_terminal or
        ...
        boolbool=shamanonosi
      else:
        action = self._agent.get_action_from_observation(observation2[0])

    if run_mode_str=='eval':
  ...
  def _run_evalrandom_phase(self, statistics):
    """Run evaluation phase.
  ...
  def _run_one_iteration(self, iteration):
    ...
    print(self._agent._sess.run(self._agent._q_values, {self._agent.state_ph: self._agent.state,  self._agent.validmoves_ph:
         [0,5,6,7,8]}))
  ...
  def _checkpoint_experiment(self, iteration):
    ...
    """
    experiment_data = self._agent.bundle_and_checkpoint("/checkpoints",
                                                        iteration)
    if experiment_data:
  ...
  def run_experiment(self):
    ...
    for iteration in range(self._start_iteration, 70):
